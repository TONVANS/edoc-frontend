// src/app/(site)/dashboard/units/page.tsx
'use client';
...
import UnitListView from '@/components/views/units/UnitListView';
import RoleGuard from '@/components/auth/RoleGuard';

export default function UnitsPage() {
  const { units, isLoading, isSyncing, fetchAll, sync } = useUnitStore();
  ...
  return (
    <RoleGuard allowedRoles={['SUPER_ADMIN']}>
      <div className="p-4 sm:p-6 lg:p-8 max-w-[1400px] mx-auto min-h-full animate-in fade-in duration-500">
        ...
        <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-5">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-white/60 backdrop-blur-md shadow-sm border border-white/80 flex items-center justify-center shrink-0">
              <Network className="text-[#185C4D] w-6 h-6" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-slate-800 tracking-tight">ຂໍ້ມູນໜ່ວຍງານ (HRM)</h1>
              <p className="text-slate-500 text-sm mt-0.5">ລາຍຊື່ໜ່ວຍງານທີ່ດຶງຂໍ້ມູນມາຈາກລະບົບ HRM</p>
            </div>
          </div>
        ...
        <UnitListView data={units} isLoading={isLoading} />
      </div>
    </RoleGuard>
  );
}
